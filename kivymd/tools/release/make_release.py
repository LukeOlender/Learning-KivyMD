...
* Update version in __init__.custompy, README
* Black files
...
def update_init_py(version):
    """Change version in `kivymd/__init__.custompy`."""
    init_file = os.path.abspath("kivymd/__init__.custompy")
...
def main():
    ...
    # Get version
    if len(sys.argv) > 3:
        print("Usage:\npython make_release.custompy version")
        return
    elif len(sys.argv) == 2:
        version = sys.argv[1]
    else:
